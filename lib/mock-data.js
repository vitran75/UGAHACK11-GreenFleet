...
export function addLocation(data) {
  const maxId = allLocations
    .filter((l) => l.id.startsWith('FD'))
    .reduce((max, l) => Math.max(max, parseInt(l.id.slice(2), 10)), 0);
  const newLocation = { ...data, id: `FD${maxId + 1}`, type: data.type || 'dealership' };
